from pyrogram import Client, filters from pyrogram.types import Message from Security import app from pymongo import MongoClient from config import MONGO_DB_URI, OWNER_ID from datetime import datetime, timedelta import random

mongo_client = MongoClient(MONGO_DB_URI) db = mongo_client.security settings_collection = db.settings security_col = db.security

MUTE_TIMES = [60, 120, 300, 600, 900, 1800]

@app.on_message(filters.group) async def enforce_security(client: Client, message: Message): if not message.from_user: return
...
async def take_action(client, message, group_id, user_id, reason): duration = random.choice(MUTE_TIMES) until_date = datetime.utcnow() + timedelta(seconds=duration)
...
async def mute_user(client, group_id, user_id, message): reason = "Message too long" await take_action(client, message, group_id, user_id, reason)
